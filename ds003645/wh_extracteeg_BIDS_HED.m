...
path2save = fullfile(fileparts(currentPath), 'event-processed'); 
...
for isubj = 2:length(datInfo)    % Loop accross subjects (Subject 0001 was taken out of the analysis)
	...
	%end
        ...
        selected_chans = find(eeg_mask | stim_chan_mask); % exclude MEG channels
...
%         EEG = pop_selectevent( EEG, 'type',[arrayfun(@num2str,[5 6 7 13 14 15 17 18 19],'UniformOutput',false) 'circle'] ,'deleteevents','on'); 
        ...
        if ~isempty(event4352)
            for ievt =1: length(event4352)
                ...
                if EEG.event(event4352_1 - 1).type == 256 || EEG.event(xevent4352_1 - 1).type == 4096
