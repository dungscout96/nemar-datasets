...
% -----------------------------------------------------|
clear
currentPath = fileparts( which('wh_extracteeg_BIDS_HED') );
path2code = currentPath;

% path2data = fullfile(currentPath, 'ds117forbids'); % Arno's path
path2data = fullfile(fileparts(currentPath), 'event-processed');

% path2bids = fullfile(currentPath, 'bids_export'); % Arno's path
path2bids      = fullfile(fileparts(currentPath), 'BIDS-EXPORT');

% path2stimfiles = fullfile(currentPath, 'faces_images'); % Arno's path
path2stimfiles = fullfile('/data/projects/BIDSfiles/wakeman-henson', 'faces_images');
...
generalInfo.BIDSVersion = 'v1.9.0';
...
% ------------------------
CHANGES = sprintf([ '2.1.0 2024-04-08\n' ...
                    ' - Fix channel montage orientation issue being rotated 90 degree to the left\n' ...
                    '2.0.2 2023-04-14\n' ...
                    ' - Added EthicsApprovals\n' ...
                    '2.0.1 2023-04-14\n' ...
                    '  - HEDVersion was updated to 8.1.0\n' ...
                    '  - BIDS version corrected to 1.8.0\n' ...
                    '2.0.0 2022-01-25\n' ...
                    '  - Add original MEG recordings - Update HED annotations - Restore to original sampling rate\n' ...
                    '1.0.0 2021-04-01\n' ...
